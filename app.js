...
// built-in asset categories, each carrying its own color/icon plus a "type" that
// drives special behavior (Commodities gets a metal picker + weight column + spot-price
// sync, Crypto gets a coin picker + live price sync; "generic" is a plain manual entry,
// same shape any user-created category gets)
const BUILTIN_ASSET_CATEGORIES = [
  {
    key: "Commodities", label: "Commodities", color: "#ffb454", type: "commodity",
    icon: `<svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M6 8h12l3 9H3z"/><path d="M6 8l1.8-3h8.4L18 8z" opacity="0.55"/></svg>`,
  },
  {
    key: "Crypto", label: "Crypto", color: "#9b7bff", type: "crypto",
    icon: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="12" cy="12" r="9"/><path d="M12 6.5l4 5.5-4 5.5-4-5.5z" fill="currentColor" stroke="none"/></svg>`,
  },
  {
    key: "Stocks", label: "Stocks", color: "#4ade80", type: "generic",
    icon: `<svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><rect x="3.5" y="12" width="4" height="8" rx="0.8"/><rect x="10" y="6" width="4" height="14" rx="0.8"/><rect x="16.5" y="9" width="4" height="11" rx="0.8"/></svg>`,
  },
  {
    key: "Cash", label: "Cash", color: "#4de8e0", type: "generic", hideIfEmpty: true,
    icon: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M21 12V7H5a2 2 0 0 1 0-4h14v4"/><path d="M3 5v14a2 2 0 0 0 2 2h16v-5"/><path d="M18 12a2 2 0 0 0 0 4h4v-4Z"/></svg>`,
  },
];

// per-item metal choice inside the Commodities category — determines which spot price
// (gold vs silver, per gram) a row's weight gets multiplied by on refresh
const METAL_OPTIONS = ["Gold", "Silver"];

// generic "tag" icon used for every user-created category
const CUSTOM_CATEGORY_ICON = `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M20.59 13.41 12 22 2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82Z"/><circle cx="7" cy="7" r="1.6" fill="currentColor" stroke="none"/></svg>`;

// colors auto-assigned to new custom categories, cycling in creation order
const CUSTOM_CATEGORY_PALETTE = ["#f472b6", "#60a5fa", "#34d399", "#fb923c", "#a78bfa", "#fbbf24", "#f87171", "#2dd4bf"];

function nextCustomCategoryColor(data) {
  const n = (data.customCategories || []).length;
  return CUSTOM_CATEGORY_PALETTE[n % CUSTOM_CATEGORY_PALETTE.length];
}

// full list of categories available right now: built-ins plus whatever the user added
function getAllCategories(data) {
  const custom = (data.customCategories || []).map((c) => ({ ...c, type: "generic", icon: CUSTOM_CATEGORY_ICON, custom: true }));
  return [...BUILTIN_ASSET_CATEGORIES, ...custom];
}

function getCategoryMeta(data, key) {
  return getAllCategories(data).find((c) => c.key === key)
    || { key, label: key, color: "#63728e", icon: CUSTOM_CATEGORY_ICON, type: "generic" };
}
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed.assets)) {
        parsed.assets = parsed.assets.map((a) => {
          if (a.category === "Gold") return { ...a, category: "Commodities", metal: "Gold" };
          if (a.category === "Silver") return { ...a, category: "Commodities", metal: "Silver" };
          return a;
        });
      }
...
function computeDerived(data) {
  ...
  const ringData = Object.entries(byCategory).map(([key, value]) => {
    const meta = getCategoryMeta(data, key);
    return { name: meta.label, key, value, pct: totalAssets ? (value / totalAssets) * 100 : 0, color: meta.color, icon: meta.icon };
  });
          ...
          ${d.ringData.map((r) => `
            <div class="legend-row">
              <span style="color:var(--text-dim);display:flex;align-items:center;gap:7px">
                <span style="display:inline-flex;color:${r.color}">${(r.icon || "").replace(/width="17" height="17"/, 'width="13" height="13"')}</span>
                ${esc(r.name)}
              </span>
...
function renderAssetsTab() {
  ...
  const categories = getAllCategories(state.data);
  ...
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:14px" id="assets-root">
      ...
      ${categories.map((cat, ci) => assetCategoryHTML(cat, rows, ci)).join("")}
      ...
      <div class="frame enter" id="new-category-form">
        <div class="corner tl"></div><div class="corner tr"></div><div class="corner bl"></div><div class="corner br"></div>
        <div class="label">New category</div>
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
          <input class="ghost-input sans" id="new-cat-name" placeholder="e.g. Real Estate, NFTs, P2P Lending" style="flex:1;min-width:200px;background:rgba(126,201,255,0.05);border-radius:4px" autofocus />
          <button class="glow-btn" id="create-category-btn">${ICON.plus} Create</button>
          <button class="icon-btn" id="cancel-category-btn">Cancel</button>
        </div>
      </div>` : `<button class="dashed-btn" id="add-category-btn">${ICON.plus} New category</button>`}
    </div>
  `;
  ...
  if (addCatBtn) addCatBtn.addEventListener("click", () => { state.addingCategory = true; renderAssetsTab(); });

  const cancelCatBtn = document.getElementById("cancel-category-btn");
  if (cancelCatBtn) cancelCatBtn.addEventListener("click", () => { state.addingCategory = false; renderAssetsTab(); });
  ...
  if (createCatBtn) {
    const submit = () => {
      ...
      const newCat = { key: uid(), label, color: nextCustomCategoryColor(state.data) };
      if (!state.data.customCategories) state.data.customCategories = [];
      state.data.customCategories.push(newCat);
      state.addingCategory = false;
      saveData(state.data);
      renderAssetsTab();
    };
    createCatBtn.addEventListener("click", submit);
...
function assetCategoryHTML(cat, rows, ci = 0) {
  ...
  if (catRows.length === 0 && cat.hideIfEmpty) return "";
  const catTotal = catRows.reduce((s, r) => s + (Number(r.currentValue) || 0), 0);
  const isCommodity = cat.type === "commodity";
  const hasCrypto = cat.type === "crypto";
  const delay = 50 + ci * 60;
  const allSelected = catRows.length > 0 && catRows.every((r) => state.selection.assets.has(r.id));

  return `
    <div class="frame enter" style="animation-delay:${delay}ms" data-category-frame="${cat.key}">
      <div class="corner tl"></div><div class="corner tr"></div><div class="corner bl"></div><div class="corner br"></div>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
        <div style="display:flex;align-items:center;gap:10px">
          <span style="display:inline-flex;color:${cat.color};filter:drop-shadow(0 0 4px ${cat.color}88)">${cat.icon || ""}</span>
          <span style="font-size:15px">${esc(cat.label)}</span>
          <span class="mono" id="cat-total-${cat.key}" data-raw-value="${catTotal}" style="font-size:12.5px;color:var(--text-dim)">${fmt(catTotal)}</span>
        </div>
        <div style="display:flex;gap:6px">
          <button class="icon-btn" data-action="add-asset" data-category="${cat.key}" title="Add ${esc(cat.label)} asset">${ICON.plus}</button>
          ${cat.custom ? `<button class="icon-btn danger" data-action="delete-category" data-category="${cat.key}" data-label="${esc(cat.label)}" title="Delete category">${ICON.trash}</button>` : ""}
        </div>
      </div>
      ...
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th style="width:26px"><input type="checkbox" class="select-all-category" data-select-all-category="${cat.key}" ${allSelected ? "checked" : ""} /></th>
            <th style="min-width:160px">Name</th>
            ${isCommodity ? `<th style="min-width:90px">Metal</th><th style="min-width:90px">Weight (g)</th>` : ""}
          ...
          <tbody data-category-tbody="${cat.key}">
...
function buildMetalSelectHTML(row) {
  const current = row.metal || "Gold";
  return `<select class="ghost-input sans" data-id="${row.id}" data-field="metal" data-type="text">
    ${METAL_OPTIONS.map((m) => `<option value="${m}" ${m === current ? "selected" : ""}>${m}</option>`).join("")}
  </select>`;
}
...
function assetRowHTML(r, cat, i = 0) {
  ...
  return `
    <tr class="enter-fast ${checked ? "row-selected" : ""}" style="animation-delay:${delay}ms" data-asset-row="${r.id}">
      ...
      ${isCommodity ? `
        <td>${buildMetalSelectHTML(r)}</td>
        <td><input class="ghost-input" data-id="${r.id}" data-field="grams" data-type="number" value="${r.grams ?? ""}" placeholder="—" /></td>
      ` : ""}
      ${hasCrypto ? `
...
function addAssetRow(category) {
  const meta = getCategoryMeta(state.data, category);
  const extra = meta.type === "commodity" ? { metal: "Gold" } : {};
  state.data.assets.push({ id: uid(), category, name: "New asset", buyPrice: 0, currentValue: 0, ...extra });
  saveData(state.data);
  renderAssetsTab();
}
...
function wireAssetsTable() {
  ...
  container.addEventListener("click", (e) => {
    ...
    if (delCatBtn) {
      const key = delCatBtn.dataset.category;
      const label = delCatBtn.dataset.label;
      const count = state.data.assets.filter((r) => r.category === key).length;
      const ok = count > 0
        ? window.confirm(`"${label}" has ${count} item${count === 1 ? "" : "s"} in it. Deleting this category will delete ${count === 1 ? "it" : "them"} too. Continue?`)
        : true;
      if (!ok) return;
      state.data.customCategories = (state.data.customCategories || []).filter((c) => c.key !== key);
      state.data.assets = state.data.assets.filter((r) => r.category !== key);
      saveData(state.data);
      renderAssetsTab();
      return;
    }
...
async function refreshLivePrices() {
  ...
  try {
    ...
    const catTypeByKey = Object.fromEntries(getAllCategories(state.data).map((c) => [c.key, c.type]));
    const coinAssets = state.data.assets.filter((a) => catTypeByKey[a.category] === "crypto" && a.coinId && a.qty != null);
    let coinPrices = {};
    ...
    state.data.assets = state.data.assets.map((a) => {
      const type = catTypeByKey[a.category];
      if (type === "commodity" && a.grams != null) {
        const pricePerGram = a.metal === "Silver" ? silverPerGram : goldPerGram;
        return { ...a, currentValue: a.grams * pricePerGram };
      }
      if (type === "crypto" && a.coinId && a.qty != null && coinPrices[a.coinId.trim().toLowerCase()]) {
